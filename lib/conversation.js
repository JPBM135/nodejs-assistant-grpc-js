...
class Conversation extends events.EventEmitter {
    ...
    sendRequest(request) {
        ...
        return this.sendRawRequest(common.mapAssistantRequestToAssistRequest(finalRequest));
    }
    end() {
    ...
    _setupEvents() {
        this._stream
            .on('data', (data) => {
            const mappedResponse = common.mapAssistResponseToAssistantResponse(data);
